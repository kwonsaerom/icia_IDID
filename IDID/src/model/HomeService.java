...
*/	}
	...
	private String makeReplyHtml(int code, ArrayList<Reply> rpList) {
		...
		for(int i=0; i<rpList.size(); i++){
			reply = rpList.get(i);
			sb.append("<td>작성자:"+reply.getRP_ID()+"</td>");
			sb.append("<td>"+reply.getRP_COMMENT()+"</td>");
			sb.append("<td style='text-align:right; margin-left:150px;'>"+reply.getRP_DATE()+"</td><hr/>");
		}
		sb.append("</tr>");
		/*sb.append("<textarea rows='5' cols='50' style='resize: none;' id='comment'></textarea><br/>");
		sb.append("<input type='button' onclick=\"javascript:reply_ajax('comment_write?code="+code+"','#reply')\" value='댓글달기'/>");*/
	...
	private String makeReviewDetail(ArrayList<Review> list, ArrayList<Reply> rpList) {
		...
		for(int i=0; i<list.size(); i++){
			...
			sb.append("<tr><td style='text-align:right;' colspan='2'><a href=\"javascript:AJ('mReview','#printP')\"><input type='button' value='목록보기'/></a><a href=\"javascript:AJ('mReview_form','#printP')\"><input type='button' value='글쓰기'/></a></td>");
			if(id.equals(ss_mId)){
