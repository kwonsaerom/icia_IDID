...
public class HomePageMove {
	...
	private String mReviewMakeList(Review review) {
		...
		sb.append("<form action='mReview_modify' method='post' enctype='multipart/form-data'><table class='table table-striped'>");
		sb.append("<input type='hidden' name='modifyCode' value='"+review.getR_code()+"'/>");
		System.out.println("reviewCode="+review.getR_code());
		if(review!=null){
			sb.append("<tr><td colspan='2'><div align='center'>제목: <input type='text' name='modify_title' value='"+review.getR_title()+"'/></div></td></tr>");
			sb.append("<tr><td colspan='2'><span align='left'>글쓴이:"+review.getR_m_id()+"</span></td></tr>");
			...
			sb.append("<tr><td colspan='2'><div align='center'>내용:<textarea name='modify_substance' rows='10' cols='52' style='resize:none;' value='"+review.getR_substance()+"'/><br/></div></td></tr>");
			sb.append("<tr><td colspan='2'>이미지 수정:<img id='image' src='image/"+review.getR_fileName()+"'/></td></tr>");
			sb.append("<tr><td><input type='file' name='modify_filename' id='modify_filename'/></td></tr>");
			sb.append("<tr><td><input type='submit' value='수정완료' /></td>");
			sb.append("<td><input type='button' onclick='back()' value='뒤로가기' /></td></tr>");
		}
	...
	private String makeReviewHtml(ArrayList<Review> list) {
		...
		Review review = new Review();
		HttpSession ss = request.getSession();
		String mId = (String)ss.getAttribute("m_id");
		...
		sb.append("<a href=\"javascript:AJ('mReview','#printP')\"><input type='button' value='목록보기'/></a></td>");
		if(mId!=null){
			//sb.append("<a href=\"javascript:AJ('mReview_form','#printP')\" id='wBtn'><input type='button' value='글쓰기' /></a></td>");
			sb.append("<td colspan='4' style='text-align:right;'><a href=\"javascript:AJ('mReview_form','#printP')\" id='wBtn'><input type='button' value='글쓰기' /></a></td>");
		}
		sb.append("</tr>");
		sb.append("</table>");
